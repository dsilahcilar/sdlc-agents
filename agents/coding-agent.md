...
If the user includes skill directives in their request (e.g., `#TDD`, `#Security !Kafka`), parse and load them:
...
# Resolve skill names to paths (for Coding Agent)
SKILL_PATHS=$(.sdlc-agents/tools/skills/resolve-skills.sh --agent coding tdd)
```
...
Read the resolved skill files and apply their patterns/guidelines during implementation.
